...
- ### mini rasa tutorial
  ...
  3. #### 定义模型配置
     ...
     配置文件`config.yml`将定义前面的NLU模块和你的模型会用到的Core元素，本次使用的是来自大神写好的组件，这里也强烈推荐大家前往学习https://github.com/GaoQ1/rasa_nlu_gq
     
NLU的配置属于决定了使用的语言和管道，Core的policies的关键词决定了模型见会使用到的策略policies
     
     如果你只是想建立一个chatbot而不想了解太多关于算法的细节，config.yml可以不用改，直接用
